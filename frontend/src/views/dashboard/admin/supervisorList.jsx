...
const SupervisorList = () => {
  ...
  const columns = [
    ...
    },
    {
      name: "coordinator",
      sortable: true,
      selector: (row) => row?.isCoordinator,
      cell: (row) => <p>{row?.isCoordinator ? "Yes" : "No"}</p>,
    },
    {
      name: "Action",
  ...
  const handleGetSupervisor = async () => {
    ...
    try {
      ...
      console.log("Error while getting Supervisor:", error);
      toast.error(error?.data?.message || "Failed to get Supervisor!");
    } finally {
      setLoader(false);
    }
  };
  ...
  return (
    <div className="w-full px-4 pb-8 ">
      <div className="bg-white w-full p-3 rounded-2xl shadow-xl">
        <h1 className="my-4 text-xl font-medium">Supervisor List</h1>
