...
const Dashboard = () => {
  ...
  const stats = [
    {
      ...
      value: scholarList?.length || "0",
      description: "Active research scholars",
      icon: <Users className="h-6 w-6 text-muted-foreground" />,
    },
    {
      title: "Supervisors",
      value: supervisorList?.length || "0",
  ...
  //handle get supervisor list
  const getSupervisorList = async () => {
    try {
      const response = await SupervisorService.GetAllSupervisor();
      if (response?.status == 200) {
        setSupervisorList(response.data?.data);
      }
    } catch (error) {
      console.error(error);
      toast.error("failed to get supervisor list");
    }
  };

  //handle get coordinator list
  ...
  //handle get scholar list
  const getScholarList = async () => {
    try {
      const response = await ScholarService.getAllScholar();
      if (response?.status == 200) {
        setScholarList(response.data?.data);
      }
    } catch (error) {
      console.error(error);
      toast.error("failed to get supervisor list");
    ...
    <div className="space-y-6">
      <div className="flex flex-col space-y-2">
        ...
        <p className="text-muted-foreground">
          Welcome back, {user?.name || "User"}! Here's an overview of the
          Research Advisory Committee system.
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Latest thesis and report submissions
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <p className="text-sm font-medium">
                    RAC Meeting Submission Deadline
                  </p>
